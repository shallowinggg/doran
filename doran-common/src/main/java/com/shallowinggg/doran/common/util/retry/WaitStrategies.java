...
public final class WaitStrategies {
     ...
     */
    public static WaitStrategy fixedWait(long sleepTime, @NotNull TimeUnit timeUnit) throws IllegalStateException {
     ...
     */
    public static WaitStrategy randomWait(long maximumTime, @NotNull TimeUnit timeUnit) {
        Assert.notNull(timeUnit, "The time unit may not be null");
     ...
     */
    public static WaitStrategy randomWait(long minimumTime, @NotNull TimeUnit minimumTimeUnit,
                                          long maximumTime, @NotNull TimeUnit maximumTimeUnit) {
        Assert.notNull(minimumTimeUnit, "The minimum time unit may not be null");
        Assert.notNull(maximumTimeUnit, "The maximum time unit may not be null");
     ...
     */
    public static WaitStrategy incrementingWait(long initialSleepTime, @NotNull TimeUnit initialSleepTimeUnit,
                                                long increment, @NotNull TimeUnit incrementTimeUnit) {
        Assert.notNull(initialSleepTimeUnit, "The initial sleep time unit may not be null");
        Assert.notNull(incrementTimeUnit, "The increment time unit may not be null");
     ...
     * @return a wait strategy that increments with each failed attempt using exponential backoff
     */
    public static WaitStrategy exponentialWait(long maximumTime, @NotNull TimeUnit maximumTimeUnit) {
        Assert.notNull(maximumTimeUnit, "The maximum time unit may not be null");
     ...
     * @return a wait strategy that increments with each failed attempt using exponential backoff
     */
    public static WaitStrategy exponentialWait(long multiplier, long maximumTime,
                                               @NotNull TimeUnit maximumTimeUnit) {
        Assert.notNull(maximumTimeUnit, "The maximum time unit may not be null");
     ...
     * @return a wait strategy that increments with each failed attempt using a Fibonacci sequence
     */
    public static WaitStrategy fibonacciWait(long maximumTime, @NotNull TimeUnit maximumTimeUnit) {
        Assert.notNull(maximumTimeUnit, "The maximum time unit may not be null");
     ...
     */
    public static <T extends Throwable> WaitStrategy exceptionWait(@NotNull Class<T> exceptionClass,
                                                                   @NotNull Function<T, Long> function) {
        Assert.notNull(exceptionClass, "exceptionClass may not be null");
        Assert.notNull(function, "function may not be null");
    ...
    public static WaitStrategy join(WaitStrategy... waitStrategies) {
        Assert.isTrue(waitStrategies.length > 0, "Must have at least one wait strategy");
        List<WaitStrategy> waitStrategyList = CollectionUtils.newArrayList(waitStrategies);
        Assert.isTrue(!waitStrategyList.contains(null), "Cannot have a null wait strategy");
    ...
    private static final class FixedWaitStrategy implements WaitStrategy {
        ...
        public FixedWaitStrategy(long sleepTime) {
            Assert.isTrue(sleepTime >= 0L, "sleepTime must be >= 0 but is " + sleepTime);
    ...
    private static final class RandomWaitStrategy implements WaitStrategy {
        ...
        public RandomWaitStrategy(long minimum, long maximum) {
            Assert.isTrue(minimum >= 0, "minimum must be >= 0 but is " + minimum);
            Assert.isTrue(maximum > minimum, "maximum must be > minimum but maximum is " + maximum + " and minimum is" + minimum);
    ...
    private static final class IncrementingWaitStrategy implements WaitStrategy {
        ...
        public IncrementingWaitStrategy(long initialSleepTime,
                                        long increment) {
            Assert.isTrue(initialSleepTime >= 0L, "initialSleepTime must be >= 0 but is " + initialSleepTime);
    ...
    private static final class ExponentialWaitStrategy implements WaitStrategy {
        ...
        public ExponentialWaitStrategy(long multiplier,
                                       long maximumWait) {
            Assert.isTrue(multiplier > 0L, "multiplier must be > 0 but is " + multiplier);
            Assert.isTrue(maximumWait >= 0L, "maximumWait must be >= 0 but is " + maximumWait);
            Assert.isTrue(multiplier < maximumWait, "multiplier must be < maximumWait but is " + multiplier);
    ...
    private static final class CompositeWaitStrategy implements WaitStrategy {
        ...
        public CompositeWaitStrategy(List<WaitStrategy> waitStrategies) {
            Assert.isTrue(!waitStrategies.isEmpty(), "Need at least one wait strategy");
    ...
    private static final class ExceptionWaitStrategy<T extends Throwable> implements WaitStrategy {
        ...
        public ExceptionWaitStrategy(@NotNull Class<T> exceptionClass, @NotNull Function<T, Long> function) {
